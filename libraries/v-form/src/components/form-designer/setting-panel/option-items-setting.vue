  ...
  <div class="option-items-pane">
    <el-radio-group
      v-if="
        ...
        (selectedWidget.type === 'ug-select' && !selectedWidget.options.multiple) ||
        (selectedWidget.type === 'async-select' && !selectedWidget.options.multiple)
      "
    ...
    <el-checkbox-group
      v-else-if="
        ...
        (selectedWidget.type === 'select' && selectedWidget.options.multiple) ||
        (selectedWidget.type === 'select-v2' && selectedWidget.options.multiple) ||
        (selectedWidget.type === 'ug-select' && selectedWidget.options.multiple) ||
        (selectedWidget.type === 'async-select' && selectedWidget.options.multiple)
      "
      v-model="optionModel.defaultValue"
    ...
    <div v-if="selectedWidget.type === 'cascader'">
      <el-button link type="primary" @click="importCascaderOptions">{{
        $t("designer.setting.importOptions")
      }}</el-button>
      <el-button link type="primary" @click="resetDefault">{{
        $t("designer.setting.resetDefault")
      }}</el-button>
    </div>
