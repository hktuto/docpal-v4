  ...
  <el-tabs v-model="firstTab">
    <el-tab-pane name="componentLib">
      ...
      <el-collapse v-model="activeNames" class="widget-collapse">
        ...
        <el-collapse-item name="5" :title="$t('designer.dataFieldTitle')">
          <draggable
            tag="ul"
            :list="dataFields"
            item-key="key"
    ...
    <el-tab-pane v-if="showFormTemplates()" name="formLib" style="padding: 8px">
      <template #label>
        <span
          ><svg-icon icon-class="el-form-template" /> {{ $t("designer.formLib") }}</span
        >
      </template>
...
export default {
  ...
  methods: {
    ...
    loadFormTemplate(jsonUrl) {
      this.$confirm(
        this.$t("designer.hint.loadFormTemplateHint"),
        ...
        .then(() => {
          axios
            ...
            .catch((error) => {
              this.$message.error(
                this.$t("designer.hint.loadFormTemplateFailed") + ":" + error
              );
            });
        })
        .catch((error) => {
