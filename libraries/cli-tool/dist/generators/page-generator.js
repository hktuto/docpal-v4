...
import { getPagesDir, getExamplePackageDir, normalizePackageName, validatePackageName, existsSync, ensureDir, readdir, stat, copyFile, writeFile, remove, readFile } from '../utils/path-utils.js';
...
async function processPackageJson(srcPath, destPath, config) {
...
async function processNuxtConfig(srcPath, destPath, config) {
    let content = await readFile(srcPath, 'utf-8');
    // Update the config for the new page
    content = content.replace('// https://nuxt.com/docs/api/configuration/nuxt-config', `// ${config.name} page configuration`);
    await writeFile(destPath, content);
}
async function createPageComponent(targetDir, config) {
