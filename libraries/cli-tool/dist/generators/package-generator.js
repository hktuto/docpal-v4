...
import { getPackagesDir, getExamplePackageDir, normalizePackageName, validatePackageName, existsSync, ensureDir, readdir, stat, copyFile, writeFile, remove, readFile } from '../utils/path-utils.js';
...
async function processNuxtConfig(srcPath, destPath, config) {
    ...
    content = content.replace('// https://nuxt.com/docs/api/configuration/nuxt-config', `// ${config.name} package configuration`);
    await writeFile(destPath, content);
}
