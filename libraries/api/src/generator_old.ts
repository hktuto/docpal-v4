...
async function generate(){
    try{
        ...
        for(let i = 0; i < endpoint.length; i++) {
            const filePath = path.resolve(process.cwd(), "./src/_generate", endpoint[i].name + ".ts")
            if(fs.existsSync(filePath)) {
                fs.unlinkSync(filePath)
            }
            const filePathJson = path.resolve(process.cwd(), "./src/_generate", endpoint[i].name + ".json")
        ...
        await Promise.all(
            endpoint.map( 
                point => {
                    ...
                    generateApi({
                        name: point.name + '.ts',
                        output: path.resolve(process.cwd(), "./src/_generate"),
                        url: point.url,
                        ...
                        fs.writeFile( path.join(__dirname,`/_generate/${point.name}.json`), JSON.stringify(finalRoute),{}, () => {
