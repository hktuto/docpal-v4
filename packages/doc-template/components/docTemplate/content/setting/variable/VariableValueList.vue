...
const emit = defineEmits<{ (e: 'update:modelValue', value: { 
  items: { label: string }[] 
  listStyle: string
  }): void }>()
...
function addItem() { items.value.push({ label: '' }); emitValue() }
function removeItem(idx: number) { items.value.splice(idx, 1); emitValue() }
function emitValue() { emit('update:modelValue', { items: items.value, listStyle: listStyle.value }) }
</script>
