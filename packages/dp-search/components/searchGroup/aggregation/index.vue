...
function handleInputChange(value: string, key: string) {
    state._aggregation[key] = searchProvider?.aggregation.value[key].filter((item: any) => {
    return (!item.key || 
            item.key.toLowerCase().includes(value.toLowerCase())) ||
            (!!item.value && item.value.toLowerCase().includes(value.toLowerCase()))
    })
}
watch(() => searchProvider?.aggregation.value, (newValue: any, oldValue) => {
    console.log
    if(!newValue) {
        state._aggregation = {}
        return
    ...
    console.log(state.filters, 'setDefaultFilter')
    })
}, {
    immediate: true
})
defineExpose({
    clear, getData, setDefaultFilter
})
</script>
