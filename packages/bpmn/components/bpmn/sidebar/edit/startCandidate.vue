...
function candidateGroupChanged(newVal: string) {
    ...
    if(group) {
        data.data['attr_flowable:candidateStarterGroups'] = group.id;
    }else {
        delete data.data['attr_flowable:candidateStarterGroups'];
    }
    node.setData({
        ...data,
...
function refreshData(){
    ...
    candidateGroup.value = data.data['attr_flowable:candidateStarterGroups'] || '';
    ...
    <div class="itemContainer">
        <ElForm label-position="top" label-width="100px" size="small">
            <ElFormItem label="Start Candidate Group">
                  <ElSelect v-model="candidateGroup" placeholder="Select Group" :disabled="editorProvider.readonly.value" filterable clearable @change="candidateGroupChanged">
