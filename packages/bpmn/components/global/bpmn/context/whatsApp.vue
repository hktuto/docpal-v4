...
async function template_nameChanged(newVal:string){
    ...
    const {data:templateDetail} = await adminApi.api.getMessageTemplateDetailsId(detailItem.id)
...
<div class="formContainer">
    ...
    <ElForm label-position="top" label-width="100px" size="small">
        <ElFormItem label="Template Name">
            <ElSelect v-model="template_name" placeholder="Select Template" remote filterable :remote-method="searchTemplte" @change="template_nameChanged">
                <ElOption v-for="item in allTemplate" :key="item.id"  :label="item.templateName" :value="item.templateName" :disabled="editorProvider?.readonly.value"/>
            </ElSelect>
        </ElFormItem>
        <!-- variable -->
        <template v-if="template_name">
            <ElFormItem v-for="(item, index) in variables" :key="index" :label="item.attr_name">
                <ElSelect v-model="item.cdata" placeholder="Select Variable" filterable clearable @change="(val:any) => variableChanged(item,val)">
                    <ElOption v-for="option in allFields" :key="option.attr_id" :label="option.attr_name" :value="option.attr_id" :disabled="editorProvider?.readonly.value" />
                </ElSelect>
            </ElFormItem>
        </template>
    </ElForm>
