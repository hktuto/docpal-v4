// worker/db/sqlite.shared-worker.ts
...
async function ensureSqlite() {
  ...
  sqlite3 = await initSqlite({ locateFile: (f) => f === "sqlite3.wasm" ? "./sqlite3.wasm" : f });
  console.log("sqlite3", sqlite3.version.libVersion);
  console.log(self.crossOriginIsolated);
}
async function ensureDb() {
  await ensureSqlite();
  if (db) return;
  db = sqlite3 && sqlite3.opfs ? new sqlite3.oo1.OpfsDB("docpal.sqlite") : new sqlite3.oo1.DB("file:///offline-db.sqlite", "ct");
