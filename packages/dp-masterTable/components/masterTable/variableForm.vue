<template>
  <div v-loading="state.loading">
    <FormVariablesRenderer ref="FormVariablesRendererRef" @formChange="handleFormChange" />
...
async function turnFields(fields: any, initData: any, flexible: boolean = false) {
  ...
  fields.forEach(async (item: any) => {
    if (!props.ignoreList.find((iItem: any) => iItem === item.columnName)) {
      ...
      if (initData && initData[_item.name]) _item.options.defaultValue = initData[_item.name]
      if (item.relationTable) {
        state.relationfieldsDetail[_item.name] = { ...item }
        if(initData && initData[`RelationTable_${_item.name}`]) state.relationfieldsDetail[_item.name].tableId = initData[`RelationTable_${_item.name}`]
        if(initData && initData[`RelationRecordId_${_item.name}`]) state.relationfieldsDetail[_item.name].recordId = initData[`RelationRecordId_${_item.name}`]
  ...
  async function getUserGroup(field: any): Promise<any> {
    try {
      ...
      field.options.optionItems = [
        { label: $i18n.t('user_groups'), options: _groups },
        { label: $i18n.t('user_users'), options: _users },
      ]
      field.options.filterable = true
      setField(field)
    } catch (error) {
...
async function handleRelationField(fieldName: string) {
  ...
  async function handleRenderRelationField(initData: any = {}) {
    ...
    const isContinue = endPoint === 'admin' ? 
                      true :
                      initData.id ? 
