...
function handleAiUpload(content) {
  if (content.uploadId) {
    const message =
      content["upload status"] === "FAIL"
        ? "ai.uploadCompleteAndAIFail"
    ...
    const requetUpload = uploadState.value.uploadRequestList.find(
      (item) => item.uploadAiId === content.uploadId
    );
...
onBeforeUnmount(() => {
  messageHandlers.value = messageHandlers.value.filter(
    (item) => item.name !== "localMessageDialog"
  );
});
</script>
