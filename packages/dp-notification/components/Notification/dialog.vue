...
<el-dialog v-model="state.visible" class="scroll-dialog" :title="unreadCount > 0 ? `${$t('notifications')}(${unreadCount})` : $t('notifications')"
    ...
    <div class="infinite-container">
        <div
            ...
            <el-checkbox-group v-model="state.checkedNotis" class="checkboxGroup" @change="handleCheckedNotisChange">
...
<!--                        </div>-->
                        ...
                        <el-button v-if="isRoute(item)" type="primary" size="small" @click="goRoute(item)" >{{$t('jump_to_view')}}</el-button>
                      </div>
                    </div>
...
// #region module: route
    ...
    async function goRoute(item) {
        ...
        if (item.content.documentId) {
            ...
            else { router.push({path: `/workflow/${item.content.processInstanceId}?state=complete`}) }
        }
      state.visible = false
    }
// #endregion
