...
function initDrag() {
  ...
  bindDragEvents(interactInstance.value, emit.bind(this));
}

function initResize() {
  interactInstance.value?.resizable(resizeOption.value);
  bindResizeEvents(interactInstance.value, emit.bind(this));
}

function initDrop() {
  interactInstance.value?.dropzone(dropOption.value);
  bindDropEvents(interactInstance.value, emit.bind(this));
}
function initGesture() {
  interactInstance.value?.gesturable(gestureOption.value);
  bindGestureEvents(interactInstance.value, emit.bind(this));
}
function reset() {
  destroy();
  init();
...
watch(
  () => [draggable, resizable, gesturable, droppable],
  (val, oldVal) => {
    if (val !== oldVal) {
      reset();
    }
