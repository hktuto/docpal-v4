<template>
  <div v-if="hold">
    ...
    <el-popover v-else-if="hold.status === 'P' || hold.status === 'L'" :visible="state.dVisible" placement="bottom" :width="250">
      <div v-if="hold" v-click-outside="onClickOutside">
        ...
        <el-row :gutter="10" v-if="(hold.status === 'P' && userId !== hold.applyApprovedBy) || (hold.status === 'L' && userId !== hold.removeApprovedBy)">
          <el-col :span="10">
    ...
    <BrowseActionsButton v-else-if="hold.status === 'A'" id="shareActionButton" :label="svgContent">
      <SvgIcon class="hd-unlock-img" src="/icons/file/unlock.svg" round :content="svgContent" @click="handleRemoveHold"></SvgIcon>
    </BrowseActionsButton>
