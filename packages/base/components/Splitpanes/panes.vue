...
function close(ignoreTempSize = false){
  if(state.size && state.size > state.minSize){
...
watch(() => props.size, (newValue) => {
  ...
  // check if size is smaller than minSIze + 30 toggle to close
  if(state.opened && newValue < state.minSize + 3){
    state.opened = false
    close()
  }
  if(!state.opened && newValue > state.minSize + 2){
    state.opened = true
    emits('update:size', newValue)
    emits('toggled')
  }
  state.size = newValue
})
