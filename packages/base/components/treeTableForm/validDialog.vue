...
async function handleOpen(errorData: any) {

  state.errorList = Object.keys(errorData).reduce((prev: any, key: any) => {
    errorData[key].forEach((item: any) => {
      prev.push(item)
    })
    return prev
  }, [])
  if (state.errorList.length === 0) {
    ElMessage.success(t('Data verification passed'))
  } else {
    state.visible = true
  }
}
