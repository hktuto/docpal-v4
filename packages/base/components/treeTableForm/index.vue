    ...
    <div class="dp-table-container">
        ...
        <el-form ref="FormRef" class="table__content"
            ...
            <el-table
                ...
                v-bind="_options"
                :header-cell-style="{'background': 'var(--app-grey-800)'}"
...
const FormRef = ref()
function classComputed(col:any) {
...
// #region Event Handling
    // 按钮组事件
    const handleAction = (command: any, row: any, index: number) => {
...
// #region module: form event
    ...
    function getPureTree (tableData:any) {
    ...
    async function getFlatData (idKey: string = 'id', parentKey: string = 'parentId') {
        ...
        if (!valid) return false
        return Object.keys(state.formData).reduce((prev:any, key:any) => {
            const _item:any = deepCopy(state.formData[key]) as any
    ...
    function getFormRule () {
        ...
        Object.keys(props.treeTableFormRule).forEach(key => {
            Object.keys(state.formData).forEach(mapKey => {
                if(props.treeTableFormRule[key]){
    ...
    function handleId (list:any, dpRowParentId ='') {
    ...
    function handleAddChild(row:any, rowIndex:any) {
        if (Object.keys(state.unitRow).length === 0) getMeta()
    ...
    function handleDelete(row:any) {
        ...
        const index = childList.findIndex((item:any) => item.dpRowId === row.dpRowId)
