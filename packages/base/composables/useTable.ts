...
export const useTableHelper = () => {
    ...
    const tableFilters = <any>{
        dateFormat: {
            ...
            fun(time:string = '', params:any  = { format: 'YYYY-MM-DD HH:mm' }){
                if(!time) return '-'
        ...
        i18n: {
            ...
            fun(value:any, params: any){
                if(params.prefix) return t(params.prefix + value)
                return t(value)
            }
        },
        duration: {
        ...
        unit: {
            ...
            fun(value:any, params: any){
                if (params.decimal > -1) value = Number(value).toFixed(params.decimal)
        ...
        if(!_col.formatList || _col.formatList.length === 0) return getProp(_row, _col.prop)
            ...
            const result = _col.formatList.reduce((prev: string, item:any) => {
                if (item.joiner) { // 连接符
                    ...
                    if (!value) prev += ''
                    else prev = prev + formatFuns[item.formatFun](value, deepCopy(item.params) )
