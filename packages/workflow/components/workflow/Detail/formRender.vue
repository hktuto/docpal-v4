...
// #region module: set
    ...
    async function handleData(data: any) {
        ...
        async function revert(key: string, value: string) {
            if (arrWidgetKeys.find((item: any) => item.name === key))
            ...
            else if (selectWidgetKeys.find((item: any) => item.name === key))
...
// #region module: get
    ...
    async function getFormData (needValidation = true, onlyWritable = false) {
        ...
        else formData = await FormRendererRef.value.vFormRenderRef.getFormData().then((res: any) => {
            return res
        }).catch((error: any) => {
            return false
        })
    ...
    function getWidgetNames (widgetNames: string [], checkMultiple: boolean = false, checkNuxeo: boolean = false) {
        ...
        return [...containerWidgets, ...fieldWidgets].reduce((prev,item) => {
            if(widgetNames.includes(item.type)) {
                if (checkMultiple) {
                    if (item.field.options.multiple) prev.push({
                        name: item.name
                    })
                }
                else prev.push({
