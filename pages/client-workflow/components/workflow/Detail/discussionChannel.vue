...
async function handleAddComment (text: string, callback: Function) {
    ...
    const res = await clientApi.api.postWorkflowCommentProcess(param)
    callback()
    if (!res) return
    await getCommentList()
}
async function getCommentList () {
    const res = await clientApi.api.getWorkflowCommentProcess({processInstanceId: props.id}).then(res => res.data || [])
    if (!res) return
