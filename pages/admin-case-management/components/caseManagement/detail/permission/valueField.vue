...
<el-date-picker
  ...
  @change="handleChange"
/>
<el-input v-else-if="type === 'input'" v-model="state.value" :placeholder="$t('tip.input')" clearable 
  @change="handleChange"
/>
<el-input-number v-else-if="numberTypes.includes(type)" v-model="state.value" v-bind="state.precisionSetting" controls-position="right" :placeholder="$t('tip.input')" clearable 
  @change="handleChange"
/>

<el-switch  v-else-if="type === 'boolean'" v-model="state.value" 
  @change="handleChange"
/>
<div v-else-if="type === 'number-range'"  class="range-input">
  <el-input-number v-model="state.value[0]" v-bind="state.precisionSetting" controls-position="right" :placeholder="$t('tip.input')" clearable 
    @change="(value: number)=>handleChange([value, state.value[1]])"
  />
  -  
  <el-input-number v-model="state.value[1]" v-bind="state.precisionSetting" controls-position="right" :placeholder="$t('tip.input')" clearable 
...
<div v-else-if="type === 'date-range'" class="range-input">
  <el-date-picker
    ...
    type="date"
    format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" default-time="2000-01-01 00:00:00"
  ...
  <el-date-picker
    ...
    type="date"
    format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" default-time="2000-01-01 23:59:59"
    @change="(value: string) => handleChange([state.value[0], value])"
  />
</div>
<el-select-v2 v-else-if="type === 'master_table'" v-model="state.value" :options="state.recordOptions" filterable clearable
  @change="handleChange"> 
</el-select-v2>
<el-select-v2 v-else-if="type === 'user_group'" v-model="state.value" :options="state.userOptions" filterable clearable
  @change="handleChange"> 
</el-select-v2>
...
watch(() => props.config, (newValue, oldValue) => {
  if(!oldValue || newValue.__cdata !== oldValue.__cdata) {
    if(isJSON(newValue.__cdata) && (JSON.parse(newValue.__cdata) instanceof  Object)) {
      ...
      let cDataArr = ['', '']
      if(cData.start) cDataArr[0] = ['number', 'float'].includes(newValue.type) ? Number(cData.start) : cData.start
      if(cData.end) cDataArr[1] = ['number', 'float'].includes(newValue.type) ? Number(cData.end) : cData.end
      setValue(cDataArr)
    }
    else {
      let _cData = newValue.__cdata
      if(['number', 'float'].includes(newValue.type)) _cData = Number(_cData)
      else if(newValue.type === 'boolean') _cData = _cData === 'false' ? false : Boolean(_cData)
      setValue(_cData)
    }
  ...
  if(newValue.type === 'master_table') getMasterTableOptions(newValue.masterTable, newValue.displayField)
