  ...
  <VxeGrid ref="tableRef" v-bind="tableConfig" v-on="tableEvent">
    <template #toolbar_buttons>
      <div class="flex-x-start">
        ...
        <Icon
          ...
          @click="tableDialogRef.handleOpen({ name: state.name })"
        ></Icon>
      </div>
    ...
    <template #columnName="{ row }">
      <div class="masterTable-columnName">
        ...
        <div
          v-if="row.unique && !isDefault(row)"
          class="column-dynamic"
          style="--column-color: #0099ff"
        >
          <div class="column-dynamic-point"></div>
          {{ $t("marsterTable.unique") }}
        </div>
        <div
          v-else-if="row.required && !isDefault(row)"
          class="column-dynamic"
          style="--column-color: #7b61ff"
        >
          <div class="column-dynamic-point"></div>
...
function handleSingleSchemaAdd(row: any) {
  schemaDialogRef.value.handleOpen(
    {unique: row.unique, dataType: row.value},
    {disabledUnique: !row.unique}
  );
}
...
async function handleSubmit() {
  ...
  if (tableConfig.data.length === 6) {
    ElMessage.error(t('tip.masterTable.noValidData'));
...
function handleAddSchama(schema: any) {
  ...
  if (index !== -1) {
    ElMessage.error(t('tip.duplicate'));
    setTimeout(() => {
      checkDisabledUniqueAndOpenSchemaDialog(schema);
    }, 1000);
  } else {
    routerProvider?.message.success(t('masterTable_createAddColumnSuccessMsg', {
...
function handleUpdateSchama(schema: any) {
  ...
  const curIndex = tableConfig.data.findIndex(
    (item: any) => item.fieldName === state.curRow.fieldName
  );
  const duplicateIndex = tableConfig.data.findIndex(
    (item: any, index: number) =>
      item.fieldName === schema.fieldName && index !== curIndex
  );
  if (duplicateIndex !== -1) {
    ElMessage.error(t('tip.duplicate'));
    setTimeout(() => {
