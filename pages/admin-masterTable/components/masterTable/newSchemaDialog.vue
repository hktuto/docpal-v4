        ...
        <div class="dialog-footer">
            <div class="flex-x-start">
                <div v-if="type !== 'again'">
                    ...
                    <el-switch v-model="state.form.unique" :disabled="state.options.disabledUnique" 
                        @change="handleUniqueChange"/>
                </div>
                <div>
                    <span class="el-icon--left">{{ $t('form_isRequire') }}</span>
                    <el-switch v-model="state.form.required" :disabled="state.options.disabledRequired"
...
// #region module: form change
    ...
    function handleAgainFormChange({fieldName, formModel, newValue, oldValue}: any) {
        ...
        state.options.disabledRequired
        const field: any = { 
            id: "defaultValue", 
            label: "defaultValue", 
        ...
        const defaultDataType = [ 'varchar:4000','varchar:255', 'bigint', 'decimal', 'timestamp', 'boolean']
        if(relationFields.includes(fieldName) && formModel.dataType === 'relation' && formModel.relationTable && formModel.relationField && formModel.displayField) {
