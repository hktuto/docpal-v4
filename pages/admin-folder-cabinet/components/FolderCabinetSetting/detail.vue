  ...
  <div class="detail-container" v-loading="state.loading" :class="{'not-root': !isRoot}">
    <div style="overflow: auto;padding: 0 var(--app-padding);">
      ...
      <div style="padding: 0 var(--app-padding);">
        ...
        <FolderCabinetSettingPermission :id="state.setting.id" :tableData="state.acls"
    ...
    <FolderCabinetSettingWorkflowDialog ref="WorkflowDialogRef" />
  </div>
</template>
...
// #region module:
  ...
  function handleDocTypeChange(data) {
    ...
    FormVariablesRendererRef.value.init(data.metaList.reduce((prev,item) => {
      prev.push({
        id: item.metadata,
        name: item.metadata,
        label: item.metadata,
