...
describe('[client-ai-upload]AiUploadDetail', () => {
  ...
  it.skip('should handleSubmit the component correctly', async () => {
    ...
    wrapper.vm.treeRef = {
      store: {
        nodesMap: vi.fn(() => Promise.resolve(nodeMap))
      }
    }

    const checkfileexistResponse = [
      { id: 1, docName: 'a' },
      { id: 2, docName: 'b' }
    ]
    vi.spyOn(clientApi.api, 'postNuxeoDocumentCheckfileexist').mockResolvedValue({ data: checkfileexistResponse })
    const batchconfirmResponse = {}
    vi.spyOn(clientApi.api, 'postNuxeoDocumentBatchconfirm').mockResolvedValue({ data: batchconfirmResponse })
    vi.spyOn(wrapper.vm, 'checkFailedListExist').mockResolvedValue(false)

    wrapper.vm.MetaFormRef = {
      checkMetaValidate: vi.fn(() => Promise.resolve(true))
    }
    ...
    expect(wrapper.vm.checkFailedListExist).toHaveBeenCalledWith(fileConfirmDTOList)
