...
<!--        <UploadStructurePreview class="main-right" ref="previewRef" />-->
        <div class="upload-footer flex-x-between">
            <div class="space"></div>
            <div>
                <el-button :loading="state.submitLoading" :disabled="state.retryLoading" type="danger" @click.native="handleDiscard">{{$t('ai.cancelPatch')}}</el-button>
                ...
                <el-button v-if="state.status === 'Error'" :loading="state.retryLoading" :disabled="state.submitLoading" type="primary" @click.native="handleRetry" >{{ $t('ai.retryAiLoading')}}</el-button>
                <el-button :loading="state.submitLoading" :disabled="state.retryLoading" type="primary" @click.native="handleSubmit">{{$t('dpButtom_confirm')}}</el-button>
            </div>
        </div>
    </main>
</div>
...
async function handleDiscard () {
    const action = await ElMessageBox.confirm(`${$i18n.t('msg_confirmWhetherToCancel')}`,{
        confirmButtonText: $i18n.t('dpButtom_confirm'),
        cancelButtonText: $i18n.t('common_close')
    }).catch((action) => { return action })
    if(action !== 'confirm') return
...
async function handleSubmit () {
    ...
    try {
        ...
        const res = await clientApi.api.postNuxeoDocumentBatchconfirm({
            userId:userId.value,
            uploadId: id,
            fileConfirmDTOList
        })
...
async function checkFailedListExist(fileConfirmDTOList: any[]): Promise<boolean> {
    const checkFailedList = await clientApi.api.postNuxeoDocumentCheckfileexist({
        uploadId: id,
