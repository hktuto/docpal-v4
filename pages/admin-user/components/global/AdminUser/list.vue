...
provide(userProviderKey, {
    getAllUsersApi: async(params:any) => {
        const res: any = await adminApi.identityNuxeo.postGetallusers({ ...params, ...userTableFilter })
...
<template>
    <UserTable ref="tableRef" :condition="userCondition" @filter-change="handleFilterChange" ></UserTable>
</template>