<template>
<el-dialog v-model="state.visible" :title="state.editMode ? $t('easyForm.actionsEdit') : $t('easyForm.actionsAdd')"
  class="scroll-dialog"
  append-to-body 
  ...
  <el-form ref="FormRef" style="--icon-size: 1.2rem;" label-position="top" :model="form">
    <el-form-item :label="$t('docType_label')" prop="actionName" 
      :rules="[inputRule]">
      <el-input v-model="form.actionName" :placeholder="$t('render.hint.fieldRequired')" />
    </el-form-item>
    <el-form-item :label="$t('easyForm.type')" prop="actionType"
      :rules="[selectRule]">
      <el-select-v2 v-model="form.actionType" :options="typeOptions" filterable  @change="handleChange" />
    </el-form-item>
    <template v-if="form.actionType">
      <el-divider content-position="left">{{ $t('masterTable.setting') }}</el-divider>
      <el-form-item :label="$t('easyForm.'+ form.actionType +'TemplateKey')" prop="actionKey"
        :rules="[selectRule]">
        <el-select-v2 v-model="form.actionKey" :options="state.templateList" filterable  @change="handleKeyChange" />
      </el-form-item>
    </template>
...
// #region module: action
  async function handleSubmit () {
    ...
    try {
      ...
      const action =  await adminApi.api.postFormDesignSaveFormresultAppend(params).then(res => res.data)
...
// #region module: change
  ...
  async function getWorkflow() {
    if(workflowList.length === 0) {
      const res = await adminApi.api.getFormDesignProcessDefinitions().then(res => res.data);
  ...
  async function getCase() {
    if(caseList.length === 0) {
      const res = await adminApi.api.postCaseTypesPage({
        pageSize: 9999
      }).then(res => res.data)
...
// #region module: keyChange
  async function handleKeyChange(value: string, isInit = false) {
    ...
    state.targetList.forEach(item => {
      ...
      if(index === -1) form.value.dataMapping.push({
        source: '',
        target: item.value,
      })
    });
  ...
  async function getEmailProps(id: string) {
    try {
      const options = await adminApi.api.getTemplateEmailTemplateId(id).then(res => res.data)
