...
function getFileName(name: any) {
  const result =
    "[" +
    formatDate(state.detail.submittedDate, "YYYYMMDD-HHmm") +
    " " +
    state.detail.email +
    "]" +
    name;
...
async function handleDblclick(row: any) {
  ...
  if (state.selectedRow.properties)
    await MetaFormRef.value.setData(state.selectedRow.properties);
  setTimeout(() => {
    state.loading = false;
  }, 1000);
...
async function handleDownload(file: any) {
  try {
    ...
    const blob: any = await clientApi.api.getWorkflowTaskAttachment({attachmentId: file.id}, {
      format: 'blob'
    });
    ...
    <div class="middle-bottom">
      <el-form ref="formRef" :model="state.selectedRow" label-position="top">
        ...
        <el-form-item :label="$t('dpDocument_fileType')" prop="documentType">
          <el-select
            v-model="state.selectedRow.documentType"
            filterable
