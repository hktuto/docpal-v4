...
export const useUploadAIStore = () => {
    ...
    async function createUploadRequest(doc: any, files: any[]) {
        ...
        const breadcrumbList = await clientApi.api.postNuxeoDocumentBreadcrumb({idOrPath:doc.path}).then(res => res.data) || []
        const path = breadcrumbList.reduce((prev, item) => {
            if(prev !== "/") prev += '/'
            prev += item.name
            return prev
        }, "/")
        
        const uploadAiId = await clientApi.api.postNuxeoDocumentSaveuploadfileoverviewDeprecate({
            userId: userId.value,
    ...
    async function handleCreateDocument (doc: any, parentPath: string, uploadRequestItem: uploadRequest) {
        ...
        try {
            ...
            if(doc.isFolder) {
                ...
                result = await clientApi.api.postNuxeoDocumentUploadtempfile({uploadTempFileRequestStr:""}, formData as any, {
